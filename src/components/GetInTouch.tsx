...
export default function GetInTouch({ formTitle = "Contact Form", formFields = defaultFormFields, sectionId }: GetInTouchProps) {
  ...
  const handleInputChange = (name: string, value: string | File | null) => {
    ...
    // Clear submit status when user makes changes (but not immediately after submission)
    if (submitStatus && !justSubmitted) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Reset form after a short delay to ensure modal shows
      setTimeout(() => {
        setFormData({});
        setSelectedFiles({});
        ...
        // Allow status to be cleared after form reset
        setTimeout(() => {
  ...
  return (
    <>
      ...
      {(submitStatus === "success" || submitStatus === "error") && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 px-4 py-10 overflow-y-auto" onClick={closeModal}>
          <div
            className={`relative w-full max-w-md rounded-2xl shadow-2xl p-6 sm:p-8 ${
              submitStatus === "success" ? "bg-green-50 border-2 border-green-400" : "bg-red-50 border-2 border-red-400"
            }`}
            ...
            <div className="flex flex-col items-center text-center space-y-4">
              {submitStatus === "success" ? (
                <>
                  ...
                  <h3 className="text-[20px] sm:text-[24px] font-bold text-green-800" style={{ fontFamily: "'Figtree', sans-serif" }}>
                    Success!
                  </h3>
                  <p className="text-[14px] sm:text-[16px] text-green-700" style={{ fontFamily: "'Figtree', sans-serif" }}>
                    {submitMessage}
                  </p>
                </>
              ) : (
                <>
                  ...
                  <h3 className="text-[20px] sm:text-[24px] font-bold text-red-800" style={{ fontFamily: "'Figtree', sans-serif" }}>
                    Error
                  </h3>
                  <p className="text-[14px] sm:text-[16px] text-red-700" style={{ fontFamily: "'Figtree', sans-serif" }}>
                    {submitMessage}
                  </p>
                </>
              )}
              <button
                onClick={closeModal}
                className={`mt-4 w-full rounded-lg px-6 py-3 text-center text-[15px] sm:text-[16px] font-semibold text-white transition ${
                  submitStatus === "success"
